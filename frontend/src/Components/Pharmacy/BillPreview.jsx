...
function BillPreview({
    ...
    try {
      ...
      const res = await axios.post("http://localhost:5000/api/pharmacy/bill", {
        ...
        medicines: medicines.map((m) => ({
          ...
          amount: Number(m.amount) || Number(m.quantity) * Number(m.price || 0),
        })),
      ...
      alert(err.response?.data?.message || "Bill Save Failed");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      paymentObject.on("payment.failed", function (response) {
        console.log("Pharmacy Payment Failed:", response);
  ...
  return (
    <div className="pharmacy-bill-page">
      {/* ================= HEADER ================= */}

      <div className="pharmacy-bill-header">
      ...
      <div className="pharmacy-bill-card">
        {/* ================= PATIENT INFORMATION ================= */}

        <h3 className="pharmacy-bill-section-title">Patient Information</h3>

        <div className="pharmacy-bill-grid">
          <div className="pharmacy-bill-form-group">
            <label>UHID Number</label>

            <input type="text" value={patient?.patientUHID || ""} readOnly />
          </div>

          <div className="pharmacy-bill-form-group">
            <label>Patient Name</label>

            <input type="text" value={patient?.patientName || ""} readOnly />
          </div>

          <div className="pharmacy-bill-form-group">
            <label>Doctor Name</label>

            <input type="text" value={patient?.doctor || ""} readOnly />
          </div>
        </div>

        {/* ================= MEDICINES ================= */}

        <h3 className="pharmacy-bill-section-title">Medicine Details</h3>

        <div className="pharmacy-bill-table-wrapper">
          <table className="pharmacy-bill-table">
            <thead>
              <tr>
                <th>#</th>
                <th>Medicine</th>
            ...
            <tbody>
              {medicines && medicines.length > 0 ? (
                ...
                <tr>
                  <td colSpan="5" className="pharmacy-bill-no-medicine">
        ...
        <div className="pharmacy-bill-summary-box">
          <h3>Bill Summary</h3>

          <div className="pharmacy-bill-summary-row">
            <span>Gross Amount</span>

            <span>₹ {Number(total || 0).toFixed(2)}</span>
          </div>

          <div className="pharmacy-bill-summary-row pharmacy-bill-total">
            <span>Net Total</span>
        ...
        <h3 className="pharmacy-bill-section-title">Payment Details</h3>
        ...
        <div className="pharmacy-bill-buttons">
          <button
            className="pharmacy-bill-payment-btn"
          ...
          <button className="pharmacy-bill-print-btn" onClick={handlePrint}>
            Print Bill
          </button>

          <button
            className="pharmacy-bill-back-btn"
