...
function PatientTable({ patients, query, setQuery, filter, setFilter, onSelectPatient }) {
  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return patients
      .filter((p) => {
        if (!q) return true;
        const hay = `${p.name} ${p.uHID} ${p.condition} ${p.phone}`.toLowerCase();
  ...
  const filterOptions = ["all", "Diabetes", "Cardiac", "General"];
    ...
    <div className="doctor-panel">
      <div className="doctor-panel__header">
        <div>
          ...
          <p className="doctor-panel__subtitle">Search and manage patient records</p>
        </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((p) => (
                <tr key={p.uHID}>
                  ...
                  <td>
                    <StatusBadge
                      variant={p.status === "Pending" ? "warning" : p.status === "Critical" ? "danger" : "success"}
                    >
                      {p.status}
                    </StatusBadge>
                  </td>
