...
function Doctor() {
  ...
  const doctor = useMemo(() => {
    ...
    const generalMedicineDoctor = doctorsBySpeciality.find(
      (s) => s.speciality === "General Medicine",
    )?.doctors?.[0];
    ...
    const fetchUpcomingAppointments = async () => {
      try {
        ...
        const rawAppointments =
          payload?.data || payload?.appointments || payload || [];
        ...
        setUpcomingAppointments(
          normalized.filter((x) => x.patientName || x.id),
        );
      } catch (err) {
        setUpcomingAppointments([]);
        setUpcomingAppointmentsError(
  ...
  function handleAcceptAppointment(appt) {
    alert(
      `Appointment accepted (UI placeholder): ${appt?.patientName || "patient"}`,
    );
  }

  function handleRejectAppointment(appt) {
    alert(
      `Appointment rejected (UI placeholder): ${appt?.patientName || "patient"}`,
    );
  }
  ...
  return (
    <Layout role="Doctor" setStep={setStep}>
      ...
      >
                ...
                >
                  <div className="doctor-pendingReports">
                    {[
                      ...
                      <div
                        key={r.title}
                        className="doctor-pendingReports__item"
                      >
                ...
                >
                  <div className="doctor-recentPatients" style={{ gap: 10 }}>
                    {[
                      ...
                      <div
                        key={x.title}
                        className="doctor-recentPatients__item"
                      >
                        <div
                          className="doctor-recentPatients__meta"
                          style={{ gap: 5 }}
                        >
                          <div
                            style={{
        ...
        {step !== "dashboard" && <BackToDashboard title={pageTitles[step]} />}
          ...
          <div className="doctor-appts">
            ...
            <div className="doctor-appts__side">
              <div className="doctor-calendar">
                ...
                <div
                  className="doctor-calendar__grid"
                  style={{ marginTop: 10 }}
                >
                  {[...Array(14)].map((_, i) => {
                    const isToday = i === 3;
        ...
        {step === "profile" && (
          <DoctorProfileDashboard currentDoctor={doctor} />
        )}
        ...
        {step === "schedule" && (
          <div className="doctor-panel--inner">
            ...
            <div className="doctor-hint">
              Doctor-type view (UI-level): Day-wise availability per speciality
              & doctor
            </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const firstDoctor = doctorsBySpeciality.find(
                      (x) => x.speciality === next,
                    )?.doctors?.[0];
            ...
            {(() => {
              ...
              return (
                <div style={{ marginTop: 16 }}>
                  <div
                    style={{ fontWeight: 950, color: "#0f172a", fontSize: 14 }}
                  >
                  ...
                  >
                    {activeDoctor?.availability || doctor?.availability} •{" "}
                    {activeDoctor?.clinic || doctor?.clinic}
                  </div>
                  ...
                  <div className="doctor-hint" style={{ marginTop: 14 }}>
                    Slot granularity (30-min) can be added later if backend
                    provides appointment slots.
                  </div>
                </div>
              );
            })()}
